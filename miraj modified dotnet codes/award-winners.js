...
function displayBooks(books) {
    ...
    if (!books || books.length === 0) {
        booksGrid.innerHTML = `<div class="no-books">No award winners found.</div>`;
        return;
    }
    ...
    books.forEach((book) => {
        ...
        // Navigate to product-detail.html on book card click
        bookElement.addEventListener("click", () => {
            window.location.href = `product-detail.html?id=${book.id}`;
        });

        // Add click event to "Add to Cart" button to redirect to cart.html
